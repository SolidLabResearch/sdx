  ...
  private fieldResolver =
    ...
    async (
      ...
      if (rootTypes.includes(parentType.name)) {
        ...
        source = await getGraph(location).then((quads) =>
...
async function getSubGraphArray(
  source: Quad[],
  className: string,
...
function getProperty(store: Store, subject: string, predicate: string): string {
  return store.getObjects(namedNode(subject), namedNode(predicate), null).at(0)!
    .value;
}
...
function getDirectives(
  type: GraphQLOutputType | GraphQLField<any, any, any>
): Record<string, any> {
  if (isListType(type)) {
