...
import { ShaclParserService } from '../src/services/shacl-parser.service.js';
import { capitalize, decapitalize } from '../src/util.js';

const TEST_ASSETS_SHACL_FOLDER = './test/assets/shacl';

describe('ShaclParserService', () => {
  const reader = new ShaclParserService();
  const schemaPromise = reader.parseSHACL(TEST_ASSETS_SHACL_FOLDER);

  describe('generates a GQL Schema', () => {
    ...
    it('with directive definitions', async () => {
      const schema = await schemaPromise;
    ...
    it('with a Query entrypoint', async () => {
      const schema = await schemaPromise;
    ...
    it('with a Mutation entrypoint', async () => {
      const schema = await schemaPromise;
    ...
    describe('with Mutation Types', () => {
      let schema: GraphQLSchema;
      let types: GraphQLNamedType[];
